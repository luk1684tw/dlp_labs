...
class DDPG:
    ...
    def select_action(self, state, noise=True):
        ...
        if noise:
            noise =  torch.FloatTensor(self._action_noise.sample()).to(self.device)
            action = (action + noise).cpu().tolist()
        else:
    ...
    def _update_behavior_network(self, gamma):
        ...
        with torch.no_grad():
           ...
           q_target = reward + gamma * q_next * (-(done-1))
        ...
        nn.utils.clip_grad_norm_(self._critic_net.parameters(), 1.0)
        critic_opt.step()
        ...
        critic_net.zero_grad()
        actor_loss.backward()
        nn.utils.clip_grad_norm_(self._actor_net.parameters(), 0.5)
        actor_opt.step()
