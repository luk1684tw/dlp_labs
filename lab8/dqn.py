...
class DQN:
    ...
    def select_action(self, state, epsilon, action_space):
        ...
        rand = random.random()
        if rand > epsilon:
            # Apply action decided by net.
            state = torch.FloatTensor(state).to(self.device)
            action = torch.argmax(self._behavior_net(state)).view(-1)
            return action.item()
        else:
    ...
    def _update_behavior_network(self, gamma):
        ...
        ## TODO ##
        q_exp, _ = torch.max(self._target_net(next_state), dim=1)
        
        action = action.type(torch.LongTensor).to(self.device)
        q_value = self._behavior_net(state).gather(1, action)
        q_target = (reward + gamma * q_exp.view(-1, 1) * (1 - done)).detach()
...
def train(args, env, agent, writer):
    ...
    action_space = env.action_space # 4 Actions in total.
...
def main():
    ...
    parser.add_argument('--gamma', default=.999, type=float)
