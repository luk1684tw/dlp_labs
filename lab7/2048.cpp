...
public:
	...
	virtual float update(const board& b, float u) {
		...
		// calculate u_iso for each isomorphic, where u is TD error.
		float u_iso = u / iso_last;

		// updated value to be returned.
		float val = 0;
...
protected:
	...
	size_t indexof(const std::vector<int>& patt, const board& b) const {
		...
		for (size_t i = 0; i < patt.size(); i++) {
			// Each Pattern requires 4 bits(2^1~2^11) to record it status
			idx |= b.at(patt[i]) << (4 * i);
		}
...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				...
				// set value of this move to reward of this move plus value of afterstate s'
	 ...
	 */
	void make_statistic(size_t n, const board& b, int score, int unit = 10000) {
...
int main(int argc, const char* argv[]) {
	...
	tdl.add_feature(new pattern({ 6, 7, 8, 9, 10, 11 }));
	tdl.add_feature(new pattern({ 10, 11, 12, 13, 14, 15 }));
